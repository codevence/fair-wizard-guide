...
* ``get`` = download a template project with specified template ID from FAIR Wizard
* ``list`` = list all templates (latest versions) from configured FAIR Wizard
* ``new`` = create a new template project, it launches a simple interactive wizard for template metadata
* ``package`` = create a ZIP distribution package from the local template project (ZIP is importable to FAIR Wizard via its web interface)
* ``put`` = upload the local template project to FAIR Wizard (once or continually on-change when ``--watch`` flag is used)
...
* ``DSW_API_URL`` = URL of FAIR Wizard API with which you want to communicate. Hover mouse over your profile name to find the :ref:`About<about>` section where URL is specified.
