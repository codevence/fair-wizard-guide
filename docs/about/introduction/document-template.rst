...
Document templates transform the answers from a questionnaire to a document of a specific format. The document template usually follows some standard template, such as Horizon Europe DMP, and can support different formats, such as PDF or MS Word. The formats can be any text-based format, so they can also be, for example, JSON or XML. It follows that we can easily use them to create a machine-actionable output, too.
