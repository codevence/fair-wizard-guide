...
A :ref:`user detail<user-detail>` can be opened by clicking the name of a user or by selecting :guilabel:`Edit` in the right dropdown menu for the desired row. There, a user can be also deleted via the :guilabel:`Delete` action. Users with this permission can :ref:`invite a new user<user-invite>` by clicking :guilabel:`Invite`, or :ref:`import users<import-users>` by clicking :guilabel:`Import`.
