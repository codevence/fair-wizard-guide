...
Roles define what users can do across the FAIR Wizard instance. Each user has one global role. The role contains a set of permissions, such as whether the user can manage users, configure settings, work with knowledge models, or access all projects.

Default Roles
=============

New FAIR Wizard instances include default roles for the usual workflows:
...
A role cannot be deleted if it is assigned to users. A role also cannot be deleted if it is configured as the :ref:`default role<auth-services>` for new users.
